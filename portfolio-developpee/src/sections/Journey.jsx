...
export default function Journey() {
  ...
  return <section id="journey" className="section shell border-t border-white/10">
    <Reveal><p className="kicker">{copy.journey.kicker}</p><div className="section-heading"><h2>{copy.journey.title}</h2></div></Reveal>
    <div className="relative mt-16 grid gap-10 md:grid-cols-3 lg:grid-cols-6">
      ...
      {journey.map((item, index) => <Reveal key={`${item.year}-${index}`} delay={index * .06} className="relative pl-10 md:pl-0 md:pt-10"><span className="absolute left-0 top-0 size-4 rounded-full border-4 border-ink bg-accent md:top-0"/><strong className="font-mono text-base font-semibold text-accent">{item.year}</strong><h3 className="mt-3 font-display text-2xl font-medium leading-snug lg:text-3xl">{item.title[language]}</h3><p className="mt-3 text-base leading-7 text-muted">{item.detail[language]}</p></Reveal>)}
    </div>
  </section>
}
